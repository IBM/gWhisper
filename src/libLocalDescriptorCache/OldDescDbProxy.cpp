...
void DescDbProxy::getMessages(const grpc::protobuf::FileDescriptor * parentDesc){
        ...
        //!!! Achtung hier stimmt Logik Nicht --> endlosschleife, weil immer gleiches Kind genommen wird!
        //Können Deskriptoren sich selbst referenzieren? --> hängt er sich selbt dran? Überspringe schonmal gesehene
        // Wenn letztes Element = nächstes Element, höre auf
        //TODO: Postprocessing
...
void DescDbProxy::loadDbFromFile(std::string dbFileName, std::string hostAddress, std::shared_ptr<grpc::Channel> channel){
    ...
    for (int i=0; i < dbFile.hosts_size(); i++){
        ...
        if (host.hostaddress() == hostAddress)
        {
            for (int j=0; j < host.file_descriptor_proto_size(); j++){
                ...
                descriptor.ParseFromString(host.file_descriptor_proto(j));
