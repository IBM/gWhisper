...
openssl req -passin pass:1234 -new -x509 -days 365 -key ca.key -out ca.crt -subj "/C=CA/ST=Baden-Wuerttemberg/L=Boeblingen/O=IBM/OU=Test/CN=localhost"

# Generate valid Server Key/Cert
echo Generate Server key:
openssl genrsa -passout pass:1234 -des3 -out server_key.pem 4096
echo Generate Server signing request:
openssl req -passin pass:1234 -new -key server_key.pem -out server_csr.pem -subj "/C=DE/ST=Baden-Wuerttemberg/L=Boeblingen/O=IBM/OU=Server/CN=localhost"
...
openssl req -passin pass:1234 -new -key client_key.pem -out client_csr.pem -subj "/C=DE/ST=Baden-Wuerttemberg/L=Boeblingen/O=IBM/OU=Client/CN=localhost"
